...
#  
#  You aren't sure how many bingo boards
#  a giant squid could play at once, so rather than waste time counting its
#  arms, the safe thing to do is to figure out which board will win last and
#  choose that one. That way, no matter which boards it picks, it will win
#  for sure.
#  
#  In the above example, the second board is the last to win,
#  which happens after 13 is eventually called and its middle column is
#  completely marked. If you were to keep playing until this point, the
#  second board would have a sum of unmarked numbers equal to 148 for a final
#  score of 148 * 13 = 1924.
#  
#  Figure out which board will win last. Once it
#  wins, what would its final score be?
#  
from get_input import get_input


class BingoBoard:
    def __init__(self, board_nums: list[list[int]]):
        self.nums_board = board_nums
        size = len(board_nums[0])
        self.bool_board = [[False] * size for _ in board_nums]

    def mark(self, call: int):
        for ind, row in enumerate(self.nums_board):
           for col_ind, num in enumerate(row):
               if num == call:
                   self.bool_board[ind][col_ind] = True
                   return

    @property
    def is_complete(self):
        complete_rows = any(all(i for i in j) for j in self.bool_board)
        if complete_rows:
            return True
        cols = [True] * len(self.bool_board[0])
    ...
    def score(self, call: int) -> int:
        total = 0
        for row_ind, row in enumerate(self.nums_board):
            for col_ind, num in enumerate(row):
                if not self.bool_board[row_ind][col_ind]:
                    total += num
        return total * call
        


def play_bingo(calls: list[int], boards: list[BingoBoard]) -> tuple[int, int]:
    for call in calls:
        if not boards:
            break
        winners = []
...
def main():
    given = get_input(4, 2021)
    calls, *boards_str = given.split('\n')
    calls = map(int, calls.split(','))
    
    def extract_boards(raw_input: list[str]) -> list[BingoBoard]:
        boards = []
        current_board = []
        for i in raw_input:
            if not i:
    ...
    bingo_boards = extract_boards(boards_str)
    winners = play_bingo(calls, bingo_boards)
    print("Part 1:", next(winners)[1])
