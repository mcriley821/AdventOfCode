...
def solve_segments(segments: list[list[set[str]]]) -> Iterator[dict[str, str]]:
    for pattern in segments:
        yield solve_pattern(pattern)


def solve_pattern(pattern: list[set[str]]):
    display = {k: '' for k in "abcdefg"}

    # get 'a'
    # difference of segments between 7 and 1
    display['a'] = pattern[1] - pattern[0]

    # get 'c'
    # 0, 6, 9 will be len == 6, but only 0 and 9 have both segments of 1
    for i in pattern[6:9]:
        if len(i - pattern[0]) != 4:
            display['c'] = pattern[0] - i
            break

    # get 'f'
    # leftover segment from 1 and segment 'c'
    display['f'] = pattern[0] - display['c']

    # get 'b'
    eg_segments = pattern[-1] - (pattern[1] | pattern[2])
    for i in pattern[6:9]:  # 0, 6, 9
        subset = i - pattern[1].union(eg_segments)
        if len(subset) == 1:
            display['b'] = subset
            break
    
    # get 'd'
    display['d'] = pattern[2] - (display['b'] | pattern[0])

    # get 'g'
    for i in pattern[3:6]:
        subset = i - (pattern[1] | pattern[2])
        if len(subset) == 1:
            display['g'] = subset
            break

    # get 'e'
    display['e'] = eg_segments - display['g']

    return {v.pop(): k for k, v in display.items()}


def solve_output(output: list[str], mapping: dict[str, str]) -> list[int]:
    num = 0
    for ind, pattern in enumerate(output):
        segments = ''.join(sorted(mapping[i] for i in pattern))
        num += SEGMENTS_TO_INT[segments] * (10 ** (len(output) - ind - 1))
    return num


def main():
    given = get_input(8, 2021)
    input, output = [], []
    for line in given.strip().split('\n'):
        in_, out_ = line.split(" | ")
        input.append(sorted([set(i) for i in in_.split(' ')], key=len))
        output.append(out_.split(' '))

    print("Part 1:", 
          sum(1 for i in output for o in i if len(o) in (2, 3, 4, 7)))

    total = sum(solve_output(p, m)
                for m, p in zip(solve_segments(input), output))
    print("Part 2:", total)
