...
class Point:
    ...
    def __iadd__(self, waypoint: Waypoint):
        if waypoint.direction == "forward":
            if self.v2:
                self.depth += self.aim * waypoint.steps
            self.horiz += waypoint.steps
        elif waypoint.direction == "up":
            if self.v2:
                self.aim -= waypoint.steps
            else:
                self.depth -= waypoint.steps
        else:
            if self.v2:
                self.aim += waypoint.steps
            else:
...
def main():
    given = get_input(2, 2021) 
    waypoints = [Waypoint(*i.split()) for i in given.split('\n') if i]
    start = Point(0, 0)
    for waypoint in waypoints:
        start += waypoint
    print("Part 1:", start.depth * start.horiz)

    start = Point(0, 0, v2 = True)
    for waypoint in waypoints:
        start += waypoint
    print("Part 2:", start.depth * start.horiz)
