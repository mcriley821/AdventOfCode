...
def fuel_cost_v2(distance: int) -> int:
    return ((distance + 1) * distance) // 2


def main():
    given = get_input(7, 2021)
    n = list(sorted(map(int, given.split(','))))
    median = n[len(n) // 2]
    print("Part 1:", sum(abs(median - i) for i in n))

    mean = int(sum(n) / len(n))
    means = range(mean - 5, mean + 5)
    sums = (sum(map(fuel_cost_v2, (abs(m - i) for i in n))) for m in means)
