...
def syntax_check(string: str) -> str | None:
    brackets = []
    for char in string:
        if char in OPEN:
            brackets.append(char)
            continue
        if CLOSED.index(char) == OPEN.index(brackets[-1]):
            brackets.pop()
            continue
        return 'corrupt', ERROR_SCORES[char]

    to_complete = [CLOSED[OPEN.index(i)] for i in reversed(brackets)]
    total = 0
    for bracket in to_complete:
        total *= 5
        total += CLOSED.index(bracket) + 1
    return 'complete', total


def main():
    given = get_input(10, 2021)

    syntax = [syntax_check(i) for i in given.split() if i]
    print("Part 1:", sum(i[1] for i in syntax if i[0] == 'corrupt'))

    completed = list(sorted(i[1] for i in syntax if i[0] == 'complete'))
    print("Part 2:", completed[len(completed) // 2])
