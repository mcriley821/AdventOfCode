...
def most_common(candidates: list[str]) -> int:
    counters = [0] * len(candidates[0])
    for i in candidates:
        for ind, num in enumerate(i):
            counters[ind] += 1 if num == '1' else -1
    num = 0
    for i in counters:
        num <<= 1
        if i > 0:
...
def common_at_index(candidates: list[str], index: int) -> str:
    count = sum(1 if i[index] == '1' else -1 for i in candidates)
    return '1' if count >= 0 else '0'


def uncommon_at_index(candidates: list[str], index: int) -> str:
    count = sum(1 if i[index] == '1' else -1 for i in candidates)
    return '0' if count >= 0 else '1'


def filtered_common(candidates: list[str]) -> str:
    common_candidates = candidates.copy()
    for i in range(len(candidates[0])):
        common = common_at_index(common_candidates, i)
        common_candidates = [j for j in common_candidates if j[i] == common]
        if len(common_candidates) == 1:
            return common_candidates[0]


def filtered_uncommon(candidates: list[str]) -> str:
    uc_candidates = candidates.copy()
    for i in range(len(candidates[0])):
        uc = uncommon_at_index(uc_candidates, i)
        uc_candidates = [j for j in uc_candidates if j[i] == uc]
        if len(uc_candidates) == 1:
            return uc_candidates[0]
...
def main():
    given = get_input(3, 2021)
    binary_nums = given.split()
    gamma = most_common(binary_nums)
    epsilon = bin_not(gamma)
    print("Part 1:", gamma * epsilon)
    oxygen    = int(filtered_common(binary_nums), 2)
    scrubbers = int(filtered_uncommon(binary_nums), 2)
    print("Part 2:", oxygen * scrubbers)
