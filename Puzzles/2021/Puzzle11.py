...
class Octopus:
    ...
    total_flashes: int = 0
    flashed: bool = False
    neighbors: tuple[Octopus] = dataclasses.field(
    ...
    def __bool__(self):
        return self.energy.__bool__()


def neighbors(index: int, size: tuple[int, int]) -> Iterator[int]:
    y, x = divmod(index, size[0])
    for xt, yt in product(range(x - 1, x + 2), range(y - 1, y + 2)):
        if not 0 <= xt < size[0] or not 0 <= yt < size[1]:
            continue
        idx = yt * size[0] + xt
        if idx == index:
            continue
...
def main():
    given = get_input(11, 2021).split()

    rows, cols = len(given), len(given[0])

    octopi = [Octopus(int(j)) for i in given for j in i if i]

    for i, octo in enumerate(octopi):
        octo.neighbors = tuple(octopi[i] for i in neighbors(i, (cols, rows)))
    ...
    print("Part 1:", sum(i.total_flashes for i in octopi))
